...
/// The charge duration needs no hook here: the goal charges via `CrossbowItem.getPullTime(ItemStack)`,
/// which already resolves the weapon's own `RangedConfig` pull time (see `item.CrossbowItemMixin`),
/// and the progress it is compared against (`getItemUseTime`) already runs through the haste hook in
/// `attribute.LivingEntityMixin`.
