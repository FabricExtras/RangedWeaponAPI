...
///
/// 1.20.1 delta: Forge 47 deprecates this method and rewrites every AI call site to its own
/// `getWeaponHoldingHand(LivingEntity, Predicate<Item>)`, which tests `instanceof BowItem` /
/// `instanceof CrossbowItem` and therefore already resolves custom weapons. The hook is still applied
/// on Forge (the method itself survives), where it only serves third-party callers.
